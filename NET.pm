...
sub new
{
	my ($class,$net_name,$net_rx_ok,$net_rx_err,$net_rx_drp,$net_rx_ovr,$net_tx_ok,$net_tx_err,$net_tx_drp,$net_tx_ovr,$net_frame,$net_carrier,$net_coll,$net_rx_mo,$net_tx_mo) = @_;
	my $this = {};
	bless($this, $class);
	...
	$this->{ATT_NET_RX_MO} = $net_rx_mo;
	$this->{ATT_NET_TX_MO} = $net_tx_mo;
	return $this;
}
1;
...
{
        my ($this) = @_;
        $this->{ATT_NET_RX_MO} = `ifconfig $this->{ATT_NET_NAME} | grep "RX bytes" | awk '{print \$3}'`;
	$this->{ATT_NET_RX_MO} = substr($this->{ATT_NET_RX_MO}, 1);
}

sub show_net_tx_mo
{
        my ($this) = @_;
        $this->{ATT_NET_TX_MO} = `ifconfig $this->{ATT_NET_NAME} | grep "RX bytes" | awk '{print \$7}'`;
	$this->{ATT_NET_TX_MO} = substr($this->{ATT_NET_TX_MO}, 1);
}
